  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto w-full max-w-5xl rounded-3xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 md:p-8 shadow-2xl">
        ...
        <TabsList className="mb-6 flex w-full gap-2">
          {tabConfig.map(({ key, label, icon }) => (
            <TabsTrigger
              key={key}
              value={key}
              className="flex flex-1 items-center justify-center gap-2 rounded-full px-4 py-2.5 text-sm md:text-base font-semibold"
            >
      ...
      <div className="space-y-5">
        <div className="grid gap-4 md:grid-cols-2">
          <Label className="flex flex-col gap-2 text-sm md:text-base font-semibold">
            ...
            <Input
              ...
              required
              className="text-xs md:text-sm py-2.5 px-3"
            />
          </Label>
          <Label className="flex flex-col gap-2 text-sm md:text-base font-semibold">
            To
        ...
        <Button
          type="submit"
          className="mt-3 inline-flex w-full items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-2.5 text-sm md:text-base font-semibold text-white hover:from-blue-500 hover:to-indigo-500 transition-all duration-300 hover:scale-[1.02]"
        >
